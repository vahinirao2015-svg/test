...
resource "aws_eip" "nat" {
  ...
  tags = {
    Name = "eip-${local.name_prefix}-nat"
  }
...
# Static public IPs for the internet-facing Application Load Balancer (one per AZ).
resource "aws_eip" "alb" {
  count  = length(aws_subnet.public)
  domain = "vpc"

  tags = {
    Name = "eip-${local.name_prefix}-alb-${count.index + 1}"
  }
